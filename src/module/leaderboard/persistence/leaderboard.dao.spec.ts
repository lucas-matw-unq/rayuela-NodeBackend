...
describe('LeaderboardDao', () => {
    ...
    it('should return collection name', () => {
        expect((LeaderboardSchema.statics as any).collectionName()).toBe('Leaderboards');
    });
    ...
    it('should throw if update not found', async () => {
        model.findOneAndUpdate.mockReturnThis();
        model.exec.mockResolvedValue(null);
        await expect(dao.updateLeaderboardUsers('p1', [])).rejects.toThrow(NotFoundException);
    });
    ...
    it('should remove user from leaderboard', async () => {
        const lb = { users: [{ _id: 'u1', points: 10 }, { _id: 'u2', points: 20 }] };
        model.findOneAndUpdate.mockReturnThis();
    ...
    it('should throw if delete not found', async () => {
        model.findOneAndDelete.mockReturnThis();
        model.exec.mockResolvedValue(null);
        await expect(dao.deleteLeaderboardByProjectId('p1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw if add user not found', async () => {
        model.findOneAndUpdate.mockReturnThis();
        model.exec.mockResolvedValue(null);
        await expect(dao.addUserToLeaderboard('p1', { points: 1 } as any)).rejects.toThrow(NotFoundException);
    });

    it('should throw if remove user not found', async () => {
        model.findOneAndUpdate.mockReturnThis();
        model.exec.mockResolvedValue(null);
        await expect(dao.removeUserFromLeaderboard('p1', 'u1')).rejects.toThrow(NotFoundException);
    });
});
