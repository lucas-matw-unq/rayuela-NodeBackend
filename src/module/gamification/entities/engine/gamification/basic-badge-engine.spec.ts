...
describe('BasicBadgeEngine', () => {
    ...
    describe('newBadgesFor', () => {
        it('should return badges the user does not have and matches the rule', () => {
            const rule = new BadgeRule(
                'r1', 'p1', 'Badge1', 'desc', 'img', 1, false, [], 'Cualquiera', 'Cualquiera', 'Cualquiera'
            );
            const project = {
                id: 'p1',
    ...
    describe('internal matches', () => {
        it('should match task type', () => {
            const rule = new BadgeRule('r1', 'p1', 'B1', 'd', 'i', 1, false, [], 'Type1', 'Cualquiera', 'Cualquiera');
            ...
            expect((engine as any).matchTaskType(rule, { taskType: 'Other' }, project)).toBe(false);
        });
        ...
        it('should match time interval', () => {
            ...
            expect((engine as any).matchTimeInterval(rule2, checkin, project)).toBe(true);
        });
        ...
        it('should check if user already has badge', () => {
            const user = { getGameProfileFromProject: () => ({ badges: ['B1'] }) } as any;
            const project = {
                id: 'p1',
                taskTypes: [],
                timeIntervals: [],
                areas: { features: [] },
                gamification: { badgesRules: [{ name: 'B1', previousBadges: [], taskType: 'Cualquiera', timeIntervalId: 'Cualquiera', areaId: 'Cualquiera' }] }
            } as any;
        ...
        it('should verify contributes', () => {
            ...
            expect((engine as any).verifyContributes(rule, { contributesTo: '' })).toBe(false);
        });

        it('should throw if time interval not found', () => {
            const rule = { timeIntervalId: 'Missing' } as any;
            const project = { timeIntervals: [] } as any;
            expect(() => (engine as any).getTimeInterval(rule, project)).toThrow(BadRequestException);
        });
