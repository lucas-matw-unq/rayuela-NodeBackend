...
describe('GamificationDao', () => {
    ...
    it('should throw error if badge name exists', async () => {
        ...
        model.findOne.mockResolvedValue(template);
        await expect(dao.addBadge('p1', { name: 'B1' } as any)).rejects.toThrow('Ya existe una insignia con ese nombre');
    });

    it('should throw if project not found when adding badge', async () => {
        model.findOne.mockResolvedValue(null);
        await expect(dao.addBadge('p1', { name: 'B' } as any)).rejects.toThrow('Project not found');
    });
    ...
    it('should throw if project not found when updating badge', async () => {
        model.findOne.mockResolvedValue(null);
        await expect(dao.updateBadge('b1', { projectId: 'p1' } as any)).rejects.toThrow('Project not found');
    });
    ...
    it('should get gamification by project id', async () => {
        const saved = {
            badges: [{ _id: 'b1', projectId: 'p1', name: 'N', description: 'D', imageUrl: 'I', checkinsAmount: 1, mustContribute: false, previousBadges: [], taskType: 'T', areaId: 'A', timeIntervalId: 'TI' }],
            pointRules: [{ _id: 'r1', taskType: 'T', areaId: 'A', timeIntervalId: 'TI', score: 10, mustContribute: false }]
        };
        model.findOne.mockReturnThis();
    ...
    it('should throw if project not found when adding score rule', async () => {
        model.findOne.mockResolvedValue(null);
        await expect(dao.addScoreRule('p1', { score: 10 } as any)).rejects.toThrow('Project not found');
    });

    it('should create new gamification', async () => {
        model.create.mockResolvedValue({});
        await dao.createNewGamificationFor('p1');
        expect(model.create).toHaveBeenCalledWith({ projectId: 'p1', badges: [], pointRules: [] });
    });

    it('should save move', async () => {
        const move = { checkin: { projectId: 'p1' }, gameStatus: { newLeaderboard: [] } } as any;
        await dao.saveMove(move);
        expect(mockLeaderboardDao.updateLeaderboardUsers).toHaveBeenCalledWith('p1', []);
    });
});
