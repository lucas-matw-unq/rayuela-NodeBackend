...
describe('ProjectDao', () => {
    ...
    it('should map time interval from db', () => {
        const ti = { _doc: { name: 'T1', days: [1], time: { start: '08:00', end: '12:00' }, startDate: new Date(), endDate: new Date() } };
    ...
    it('should update and handle new areas', async () => {
        ...
        const updateDto = { areas: { type: 'FeatureCollection', features: [{ properties: { id: 'new' } }] } };
        ...
        expect(callArgs.$set.areas.features.find(f => f.properties.id === 'old').properties.disabled).toBe(true);
    });
    ...
    it('should find one', async () => {
        const doc = {
            name: 'P1', areas: { features: [] }, taskTypes: [], timeIntervals: [], ownerId: 'o1'
        };
        model.findById.mockReturnThis();
        model.exec.mockResolvedValue(doc);
        gamificationDao.getGamificationByProjectId.mockResolvedValue({});
